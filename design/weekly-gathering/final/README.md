...
| `banner-blackgold.png` | Black & Gold only, further customised: the full street address under the venue name, two flanking minaret silhouettes (`minaret_group()` — a Koutoubia-style tapered shaft, blind arches, lantern and jamour finial), and "A brothers-only gathering" dropped from the footer while the site URL stays. |
