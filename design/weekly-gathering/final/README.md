...
| `banner-blackgold.png` | Black & Gold only, further customised: the full street address under the venue name, two bold flanking minaret silhouettes with their own glow (`minaret_group()` — a Koutoubia-style tapered shaft, blind arches, lantern and jamour finial), a solid glowing moon beside the left tower (`moon_group()`), and "A brothers-only gathering" dropped from the footer while the site URL stays. |
