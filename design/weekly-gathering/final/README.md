...
| `final-illuminated-red.png` / `-green.png` / `-gold.png` / `-purple.png` | Illuminated Manuscript — not a generated pattern at all: the background is a photograph of the user's own hand-painted tazhib illumination (gold-leaf scrollwork, florals), hue-shifted from its original blue/teal into a rich red and vivid green while keeping every bit of shading and brush texture, then mirrored into a tile and repeated down the page. See `design/weekly-gathering/illuminated/README.md`. Every text zone sits on its own opaque panel shaped like the manuscript's own vocabulary — a filled horseshoe-arch dome behind the hero, cusped cartouche bars (smooth elliptical-arc caps) everywhere else — rather than plain boxes. Four panel colourways, same background and layout: the panel fill is a radial gradient built from real sampled shadow/mid/highlight tones in the artwork (not a flat tint), one each in red, green, gold (dark ink instead of white text) and purple, so they can be compared side by side. | The user's own artwork (photographed, then recoloured — not generated) |
