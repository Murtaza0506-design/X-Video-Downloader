#!/usr/bin/env python3
# LAMPLIGHT GEOMETRY — four hero treatments of the venue silhouette
import math, base64, pathlib

HERE = pathlib.Path(__file__).parent
F = HERE / "fonts"

# ---------- geometry helpers (shared with build.py) ----------
def P(cx, cy, r, a):
    t = math.radians(a - 90)
...
def halo(cx, cy, R):
    """The rim and the ninety-nine tasbih marks that used to frame the seal."""
    u = R/150.0
...
STAR_URI  = mask_uri("mask-star.png")
WORD_URI  = mask_uri("mask-wordmark.png")
BLDG_ARCH_URI = mask_uri("mask-building-wide.png")
BLDG_SQ_URI   = mask_uri("mask-building-square.png")

W, H = 1200, 1800
CX = W/2
ARCH_L, ARCH_R, ARCH_BASE, ARCH_APEX = 250, 950, 840, 118

def arch_d(inset=0.0):
    l, r = ARCH_L+inset, ARCH_R-inset
    b, a = ARCH_BASE, ARCH_APEX+inset*0.9
    sh = 430+inset*0.4
    return (f'M {l},{b} L {l},{sh} Q {l},{a+95} {CX},{a} '
            f'Q {r},{a+95} {r},{sh} L {r},{b}')

def arch(inset=0.0, cls="hair-4", w=0.8):
    return f'<path d="{arch_d(inset)}" class="{cls}" stroke-width="{w}" fill="none"/>'

AYAH = "أَلَا بِذِكْرِ ٱللَّٰهِ تَطْمَئِنُّ ٱلْقُلُوبُ"

# ---------- CSS shared by every variant ----------
def base_css():
    return f'''
{FONTS}
...
.rn{{font-family:Cinzel,serif;font-size:11.5px;letter-spacing:.34em;text-indent:.34em;color:#B99A55}}
...
def frame_svg(extra_defs="", glow_cy=360, glow_r=52, extra_body=""):
    return f'''
<svg class="layer" viewBox="0 0 {W} {H}" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  <defs>
    ...
    <radialGradient id="halo" cx="50%" cy="{glow_cy/H*100:.1f}%" r="{glow_r}%">
    ...
    <linearGradient id="bldgGrad" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0%" stop-color="#F4E1AE"/>
      <stop offset="45%" stop-color="#D6AD62"/>
      <stop offset="100%" stop-color="#8C6A34"/>
    </linearGradient>
    {extra_defs}
  </defs>
...
def bottom_html():
    """Everything from the date band down — identical across all four variants."""
    return f'''
...
def page(hero_html, extra_defs="", glow_cy=360, glow_r=52, extra_svg_body="", extra_css=""):
...
<body><div class="page">
{tiles_svg()}
{frame_svg(extra_defs, glow_cy, glow_r, extra_svg_body)}
{hero_html}
{bottom_html()}
</div></body></html>'''
